...
class ATableActionButtons extends StatelessWidget {
  const ATableActionButtons({
    super.key,
    this.view = false,
    this.edit = true,
    this.delete = true,
      ...
      children: [
        if (view)
          IconButton(
            ...
            icon: const Icon(Iconsax.eye, color: AColors.darkerGrey),
          ),
        if (edit)
          IconButton(
            onPressed: onEditPressed,
            icon: const Icon(Iconsax.pen_add, color: AColors.primary),
          ),
        if (delete)
          IconButton(
